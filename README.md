...
> 对比桌面壳（Electron/Tauri 打包的桌面客户端）：本工具没有独立运行时——托盘只是**一个几十 KB 的 PowerShell 脚本 + 一个图标**，无安装包体积、无常驻内存负担，harness 一关托盘即退，不留任何后台进程。

## 功能特性

- 🪶 **极致轻量**：纯 PowerShell（核心脚本约 30KB）+ 一行 VBS，**无 Electron、无独立运行时**——对比桌面壳/桌面客户端动辄上百 MB 安装包与 100–300MB 常驻内存，本工具托盘常驻仅几 MB，还包含随包的内置用量仪表
