...
> 对比桌面壳（Electron/Tauri 打包的桌面客户端）：差别在**体积与干净度**——本工具是几十 KB 的 PowerShell 脚本 + 一个图标，无需下载运行时、没有安装包，harness 一关托盘即退，不留后台进程。**内存不是优势项**：PowerShell + WinForms 常驻实测约 120–170MB，与 Electron 桌面壳同量级（如实说明，避免误导）。

## 功能特性

- 🪶 **零安装包、零运行时**：纯 PowerShell（核心脚本约 30KB）+ 一行 VBS，不下载 Electron/运行时、不写注册表、不驻留更新器；对比桌面壳动辄上百 MB 的安装包，本工具随 npm 包交付（还含图标与内置用量仪表）
